...
class mini2d(torch.nn.Module):
    ...
    def forward(self, x_1d: SphericalTensor, x_2d: SphericalTensor):
        ...
        a = x_2d.ten.mul(fold_1d.ten.unsqueeze(1)).sum(dim=2).mul(fold_1d.ten).sum(dim=1)
        a = self.twobody_gate(a)
        new_2d_ten = self.twobody_linear(x_2d.ten).mul(a.unsqueeze(1).unsqueeze(2))
        x_2d_new = SphericalTensor(new_2d_ten, x_2d.rep_dims, x_2d.metadata, x_2d.rep_layout, x_2d.num_channels)
...
def test_train_mini2d():
    ...
    labels = torch.sin((x_2d.ten * rs_1d.unsqueeze(1) * rs_1d.unsqueeze(2)).pow(2).mean(dim=(1,2)).sqrt()).sum(1)
    ...
    for epoch in range(50):
        for iter in range(32):
            ...
            x_1d_new = SphericalTensor(x_1d.ten[batch_idx], x_1d.rep_dims, x_1d.metadata, x_1d.rep_layout, x_1d.num_channels)
            x_2d_new = SphericalTensor(x_2d.ten[batch_idx], x_2d.rep_dims, x_2d.metadata, x_2d.rep_layout, x_2d.num_channels)
            for mod in mods:
